...
public class JwtTokenFilter extends OncePerRequestFilter {
  ...
  private final String secretKey =
      "5367566B59703373367639792F423F4528482B4D6251655468576D5A71347437"; // Your secret key
  ...
  private List<GrantedAuthority> getAuthoritiesFromToken(String token) {
    ...
    return Collections.singletonList(
        new SimpleGrantedAuthority("ROLE_" + role)); // Add role as authority
  }
  ...
  private void handleException(HttpServletResponse response, String message, int status)
    ...
    response
        .getWriter()
        .write(
            "{\"message\": \"" + message + "\"}"); // Write the error message to the response body
